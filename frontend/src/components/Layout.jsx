...
import { Ticket, User, LogOut, Menu, X, ReceiptText } from 'lucide-react';
      ...
      <nav className={`bg-white sticky top-0 z-50 transition-shadow ${scrolled ? 'shadow-md' : 'shadow-sm'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="hidden md:flex items-center gap-4">
              ...
              {user && (
                <Link to="/my-tickets" className="inline-flex items-center gap-2 text-gray-600 hover:text-indigo-600 transition">
        ...
        {menuOpen && (
          <div className="md:hidden border-t bg-white px-4 py-3 space-y-2">
            ...
            {user && (
              <Link to="/my-tickets" className="flex items-center gap-2 py-2 text-gray-600" onClick={() => setMenuOpen(false)}>
                <ReceiptText className="w-4 h-4" />
                Vé của tôi
              </Link>
            )}
